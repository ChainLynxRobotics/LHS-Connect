...
import {
	...
	shortLinkSuffixValidator,
	shortLinkUrlUpdateValidator,
} from './validator';
...
export const GET: RequestHandler = async ({ url }) => {
	try {
		...
		if (!found.hash)
			return error(401, {
				message:
					'Short link has no password, and cannot be viewed. If you would still like to view/edit the info, contact us on the about page.',
			});
...
export const PATCH: RequestHandler = async ({ request, url }) => {
	try {
		...
		if (!found.hash)
			return error(401, { message: 'Short link has no password, and cannot be edited' });
...
export const DELETE: RequestHandler = async ({ url }) => {
	try {
		...
		if (!found.hash)
			return error(401, { message: 'Short link has no password, and cannot be deleting' });
