...
import { userAdminUpdateValidator } from "$lib/validation/userValidator";
...
export const PATCH: RequestHandler = async ({ locals, request, params }) => {
    ...
    try {
        ...
        if (newPermissions.has(Permission.OWNER) != oldPermissions.has(Permission.OWNER)) error(400, "Cannot set owner permission");
        if (newPermissions.has(Permission.MANAGE_USERS) != oldPermissions.has(Permission.MANAGE_USERS) && !locals.permissions.has(Permission.OWNER)) error(403, "Cannot set manage users permission");
...
export const DELETE: RequestHandler = async ({ locals, params }) => {
    ...
    try {
        ...
        if (oldPermissions.has(Permission.MANAGE_USERS) && !locals.permissions.has(Permission.OWNER)) error(403, "Cannot delete user with manage users permission");
