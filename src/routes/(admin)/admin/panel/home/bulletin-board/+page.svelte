<script lang="ts">
    ...
    const { state, undo, redo, canUndo, canRedo } = createHistoryManager(bulletinBoardData.notes.map((note, index) => ({
        id: index,
        note: note,
    })));
...
<div class="flex flex-col items-center p-4">
    ...
    <div class="flex gap-4">
        <ButtonGroup>
            <Button color="alternative" disabled={!$canUndo} on:click={undo}><UndoOutline class="w-6 h-6" /></Button>
            <Tooltip>Undo {#if !$canUndo}(unavailable){/if}</Tooltip>

            <Button color="alternative" disabled={!$canRedo} on:click={redo}><RedoOutline class="w-6 h-6" /></Button>
            <Tooltip>Redo {#if !$canRedo}(unavailable){/if}</Tooltip>
        </ButtonGroup>
...
<Modal bind:open={editModalOpen} size="md" autoclose={false}>
    <form class="flex flex-col space-y-6" action="#" on:submit={handleEditModalSubmit}>
        ...
        <div>
            <ValidatedInput bind:this={titleInput} id="title" label="Title" validatorObject={bulletinBoardSchema} visuallyRequired />
        </div>
        <div>
            <ValidatedTextarea bind:this={contentInput} id="content" label="Content" validatorObject={bulletinBoardSchema} visuallyRequired />
        </div>
        <div>
            <ValidatedInput bind:this={linkInput} id="link" validatorObject={bulletinBoardSchema} label="Link" />
        </div>
        <div class="flex gap-4">
            <Button type="button" color="alternative" on:click={()=>editModalOpen = false}>Cancel</Button>
