...
<Table divClass="relative mx-auto" striped>
    ...
    <TableBody tableBodyClass="divide-y text-center">
        {#each schedule.periods as period}
            <TableBodyRow class={`${period.emphasis ? '!bg-gray-100 dark:!bg-gray-600' : ''} ${reactive && currentlyWithinTime(period.start, period.end) ? '!border-2 dark:!border !border-yellow-200' : 'border-b last:border-b-0 dark:border-gray-700'}`}>
...
{#if schedule.desc}
    <p class="text-center text-sm opacity-75 mt-4">{schedule.desc}</p>
{/if}